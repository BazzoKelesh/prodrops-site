function initializeForm() {
  ...
  // Dev API: https://0400-143-177-218-44.ngrok-free.app/v1
  ...
  typeInput.addEventListener("input", function (e) {
    const conditionalElement = document.querySelector("input[name=organization]").closest(".input-wrapper");
    const mandatorySocialsWrapper = document.querySelector("[data-socials-mandatory]");
    const mandatorySocials = document.querySelector("[data-socials-representative-optional]");
  ...
  document.querySelector("input[name=games][data-value=Other]").addEventListener("input", function (e) {
    const conditionalElement = document.querySelector("input[name=otherGames]").closest(".input-wrapper");
    if (e.target.checked) {
      conditionalElement.classList.remove("hidden");
    } else {
      conditionalElement.classList.add("hidden");
  ...
  $("#registrationFormStep1").validate({
    rules: {
      ...
      otherGames: {
        required: function () {
          return document.querySelector("input[name=games][data-value=Other]").checked;
        },
      },
    },
    ...
    submitHandler: function (form, e) {
      if (document.querySelector("input[name=games][data-value=Other]").checked && document.querySelectorAll("input[name=games]:checked").length === 1) {
  ...
  function nextStep() {
    const newNext = document.querySelector(`#registrationFormStep${currentStep + 2}Wrapper`);
    const newCurrent = document.querySelector(`#registrationFormStep${currentStep + 1}Wrapper`);
    const newPrevious = document.querySelector(`#registrationFormStep${currentStep}Wrapper`);
  ...
  function prevStep() {
    const newNext = document.querySelector(`#registrationFormStep${currentStep}Wrapper`);
    const newCurrent = document.querySelector(`#registrationFormStep${currentStep - 1}Wrapper`);
    const newPrevious = document.querySelector(`#registrationFormStep${currentStep - 2}Wrapper`);
    if (currentStep !== 1) {
  ...
  async function submitForm() {
    ...
    try {
      ...
      document.querySelectorAll("[data-social]").forEach((e) => {
        ...
        const oauthToken = localStorage.getItem(`partner-form-token-${provider}`);
        if (oauthToken) {
      ...
      if (f3.get("tos") !== "on" || f3.get("partnersTos") !== "on") {
        throw Error("You must accept all agreements in order to become a partner");
      }
      const selectedGames = [];
  ...
  document.querySelectorAll("[data-social]").forEach((e) => {
    const provider = e.getAttribute("data-social");
    e.addEventListener("click", (e) => connectProvider({ allowConnect: true, provider }));
    e.parentElement.querySelector(".pf-social-connect-button").addEventListener("click", () => connectProvider({ provider }));
  });

  async function connectProvider({ allowConnect, provider }) {
    try {
      const existingToken = localStorage.getItem(`partner-form-token-${provider}`);
      if (existingToken) {
        if (!allowConnect) {
      ...
      window.open(api + `/oauth/${provider}/${csrfState}`, "popup", "popup=true,toolbar=false,menubar=false");
    } catch (error) {
      console.error(error);
    }
  }
  ...
  async function saveAuthToken(provider, url) {
    ...
    if (code) {
      const res = await fetch(api + `/oauth/${provider}/token/${encodeURIComponent(code)}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    localStorage.setItem(`partner-form-token-${provider}`, JSON.stringify(authData));
    detectAuthTokens();
  }
  async function detectAuthTokens() {
    ...
    providers.forEach((e) => {
      ...
      const connectButton = buttonParent.querySelector(".pf-social-connect-button");
      const statusText = buttonParent.querySelector(".pf-social-connect-status");
      if (!oauthToken) {
        statusText.innerHTML = e.charAt(0).toUpperCase() + e.slice(1) + " not connected";
        ...
        statusText.innerHTML = e.charAt(0).toUpperCase() + e.slice(1) + " connected";
